...
**The notification delivery gap (diagnosed 2026-03-31):** The MCP server's notification pipeline is correct — observer fires, debouncer runs, `send_message()` writes to stdio without error. The break is on the CC side: channel notifications are silently dropped in most REPL states (idle at prompt, mid-turn). Only a narrow window between turns delivers reliably. This is a known CC bug with multiple open issues (anthropics/claude-code#36975, #37139, #40237, #36477), regressed in v2.1.86. The feature remains "research preview" with no fix as of v2.1.88. Community workarounds include tmux send-keys nudge daemons and patched cli.js. This means Phase 2's "comments as turn signal" architecture is architecturally correct but delivery-blocked by the client. Two private API dependencies (`ServerSession.send_message()`, `Server._handle_message`) have runtime assertions and documented versions — both validated correct over real transport via file-based debug logging.

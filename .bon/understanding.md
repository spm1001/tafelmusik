...
- **Don't trust stdin EOF through wrappers.** When a wrapper process (`uv`, `npx`, `nix run`) sits between parent and child, the parent's death doesn't propagate EOF — the wrapper survives reparenting to init and holds both pipe ends alive. MCP's `stdio_server()` cleanup never fires. Fix: track the grandparent (actual session owner) via `os.kill(pid, 0)` polling. Belt-and-suspenders: in-process watchdog handles the normal case, session-start hook backstop handles watchdog failures. Neither alone is sufficient.

## Portfolio status (2026-06-09 audit)

Parked-but-useful per Sameer. All 25 open items verified still-valid against code; tfm-rohudu needs a brief rewrite before execution (its plan assumes the deleted Y.Map comment architecture — comments are HTTP/SQLite now). Marketplace fate is tracked in batterie-de-savoir (bds-rolewu). Backlog grooming deferred to the batterie reshape (bds-hifusu).

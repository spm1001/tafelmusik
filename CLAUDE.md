...
- **Comments (HTTP/SQLite, unified):** All three surfaces — MCP tools, browser, tmux — use HTTP endpoints on the ASGI server. Comments are stored in SQLite (`CommentStore` in `anchored.py`), anchored via TextQuoteSelector (quote + prefix/suffix context). Re-anchoring is lazy — computed on read via a 4-strategy cascade (exact → disambiguate → fuzzy → context recovery), not maintained on write. **Do not re-add `collect_affected`/`reanchor` calls to `edit_doc`** — that was the old Y.Map path.
  ...
  - **`flush_doc`** still calls `comments.clear_all` on the Y.Map, but this is a no-op — no surface writes Y.Map comments anymore. The wipe is dead code pending tfm-kokudo. SQLite comments survive flush.
  - **Notification paths:** All comments (browser, Claude, tmux) → HTTP POST → ASGI broadcast 0x01 → all peers. MCP server's `_handle_comment_event` processes these. Claude's own HTTP comments are filtered by author check. The Y.Map observer in MCP still runs but will never fire for new comments — dead code pending kokudo.
  - **Dead Y.Map code:** `comments.py` module, Y.Map observer in `mcp_server.py`, flush_doc's `clear_all` call. All deletable after tfm-kokudo.

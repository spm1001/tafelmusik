...
**Phase 2 — Done but unverified end-to-end (tfm-becitu, tfm-semame, tfm-nocaga, tfm-napari, tfm-wiveje).** Comment operations extracted to `comments.py`. Y.Map observer replaced Y.Text observer for notifications — document-body edits produce silence, comments are the turn signal. Drift tracking via state vector snapshots: `doc.get_update(snapshot)` byte size measures staleness, gates full-doc pushes alongside comments or on idle timeout. Three push triggers: (a) comment + high drift = full doc + comment, (b) idle 30s + high drift = automatic resync, (c) room connect = initial context push. `DRIFT_THRESHOLD` (1024B) and `IDLE_TIMEOUT` (30s) are tuning parameters — journalctl logs drift scores on every comment and idle timer fire for empirical tuning. MCP session capture moved to initialization (wiveje) — `_install_session_capture` wraps `Server._handle_message` to grab the session on the first MCP message, with the comment consumer waiting on `session_ready` Event instead of dropping comments.

**The notification delivery gap (diagnosed 2026-03-31, workarounds mapped 2026-04-01):** The MCP server's notification pipeline is correct — observer fires, debouncer runs, `send_message()` writes to stdio without error. The break is on the CC side: channel notifications are silently dropped in most REPL states (idle at prompt, mid-turn). Only a narrow window between turns delivers reliably. This is a known CC bug with multiple open issues (anthropics/claude-code#36975, #37139, #40237, #36477), regressed in v2.1.86. The feature remains "research preview" with no fix as of v2.1.89.

**Two working push paths into Claude's context exist**, both discovered empirically from CC source (`~/Repos/claude-code/src/`): (1) **Mousetrap pattern:** background Bash commands that exit produce `<task-notification>` tags injected directly into model context. (2) **asyncRewake hooks:** hooks with `asyncRewake: true` that exit with code 2 call `enqueuePendingNotification`, which injects via `wrapInSystemReminder`. Channel notifications remain architecturally correct but delivery-broken. FileChanged hooks are a dead end — their `systemMessage` goes to a transient UI notification widget (`addNotification` with 5s timeout), never model context. The CC source also reveals swarm infrastructure (teammate mailbox, SendMessage routing, coordinator mode) that mirrors aboyeur's design but is not yet publicly accessible. Two private API dependencies (`ServerSession.send_message()`, `Server._handle_message`) have runtime assertions and documented versions — both validated correct over real transport via file-based debug logging.
...
**Selection vs commenting intent:** These two gestures share the same browser primitive (highlighting text), creating a fundamental tension. After four iterations (auto-focus compose card, escape-to-edit, no-focus, revert), the resolution: compose card appears on selection as a visual hint but focus stays in the editor. Click to write a comment. The asymmetry is a browser limitation — programmatic selection works (compose→editor) but a click always sets the cursor (editor→compose). "No-auto-focus" won because it doesn't fight the browser.
...
## y-websocket silence timer

The y-websocket library has a hardcoded 30-second `messageReconnectTimeout` that kills WebSocket connections during quiet editing periods. This was the root cause of the "connection blip during comment entry" bug (tfm-lefato) — not network instability. The ASGI server doesn't echo awareness messages, so during periods with no CRDT edits the browser sees 30s of silence and disconnects. The current fix resets `wsLastMessageReceived` every 10s as a monkey-patch. The proper fix (tfm-salima) is replacing y-websocket with standalone browser sync code, mirroring the Python-side approach where we already own the protocol.

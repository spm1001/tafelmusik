...
src/tafelmusik/
  ...
  comments.py          # Y.Map comment operations + StickyIndex re-anchoring (being replaced by anchored.py)
  anchored.py          # Standalone comment system — SQLite + TextQuoteSelector anchoring (28 tests)
...
- **`flush_doc` tool:** Reads Y.Text, writes .md to docs_dir, wipes Y.Map comments (not SQLite), git commits. The flush IS the compaction — sidesteps the pycrdt-store squashing bug for file-backed rooms. SQLite comments intentionally survive flush.
...
- **Comments (two systems, transitional):** Comments are migrating from Y.Map to SQLite (`anchored.py`). Current state:
  - **MCP tools** (`add_comment`, `list_comments`, `resolve_comment`) use HTTP endpoints on the ASGI server. Comments are stored in SQLite (`CommentStore` in `anchored.py`), anchored via TextQuoteSelector (quote + prefix/suffix context). Re-anchoring is lazy — computed on read via a 4-strategy cascade (exact → disambiguate → fuzzy → context recovery), not maintained on write. **Do not re-add `collect_affected`/`reanchor` calls to `edit_doc`** — that was the old Y.Map path.
  - **Browser** still reads/writes comments via Y.Map in the CRDT (until tfm-kozada migrates it). Four Y.Map anchor fields: `anchorStart`/`anchorEnd` (StickyIndex range), `anchor` (fallback), `quote` (for re-anchoring). Frontend sorts by overlap, decorations sort by strict position.
  - **`flush_doc`** clears Y.Map comments but does NOT touch SQLite comments. After flush, `list_comments` (HTTP) still returns them. This is transitional — resolves when browser moves off Y.Map.
  - **Notification paths:** Browser comments → Y.Map observer → queue. HTTP comments (Claude, tmux) → ASGI broadcast 0x01 → `_handle_comment_event` → queue. Both feed the same consumer. Claude's own HTTP comments are filtered by author check.
